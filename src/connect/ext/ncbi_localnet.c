/* $Id: ncbi_localnet.c 186552 2010-03-22 16:11:37Z lavr $
 * ===========================================================================
 *
...
static void s_LoadLocalIPs(void)
{
    ...
    if (!filename  ||  !(fp = fopen(filename, "r"))) {
        if (filename) {
            CORE_LOGF_ERRNO_X(2, errno != ENOENT ? eLOG_Error : eLOG_Trace,
                              errno, ("Cannot load local IPs from `%s'", buf));
        }
        CORE_LOG_X(3, eLOG_Trace, "Using default local IPs");
        assert(sizeof(s_LocalIP) >= sizeof(kLocalIP));
        memcpy(s_LocalIP, kLocalIP, sizeof(kLocalIP));
        return;
    }

    CORE_LOGF_X(4, eLOG_Trace, ("Loading local IPs from `%s'", filename));
    ...
    do {
        ...
        len = strcspn(c, " \t");
        if (c[len]) {
            size_t tail = len + strspn(c + len, " \t");
            if (c[tail]) {
                CORE_LOGF_X(5, eLOG_Error,
                            ("Skipping noisy IP spec at line %u: `%s'",
                             lineno, c));
                continue;
            }
            c[len] = '\0';
        }
        if (!NcbiParseIPRange(&local, c)) {
            CORE_LOGF_X(6, eLOG_Error,
                        ("Skipping invalid IP spec at line %u: `%s'",
                         lineno, c));
            continue;
        }
    ...
    CORE_LOGF_X(9, eLOG_Trace, ("Done loading local IPs, %u line%s, %u entr%s",
                                       ...
                                       const char* const* tracking_env)
{
    ...
    if ( !tracking_env ) {
        ...
        CORE_LOGF_X(10, eLOG_Note, ("Getenv(`%s') = %s%s%s", name,
                                    result ? "\""   : "",
                                    result ? result : "NULL",
                                    result ? "\""   : ""));
#endif /*_DEBUG && !NDEBUG*/
        ...
        for (str = tracking_env;  *str;  ++str) {
            if (strncasecmp(*str, name, name_len) == 0
                &&  (*str)[name_len] == '=') {
                result = &(*str)[name_len + 1];
                break;
            }
        }
#if 0/*defined(_DEBUG) && !defined(NDEBUG)*/
        CORE_LOGF_X(11, eLOG_Note, ("Tracking `%s' = %s%s%s", name,
                                    result ? "\""   : "",
                                    result ? result : "NULL",
                                    result ? "\""   : ""));
#endif /*_DEBUG && !NDEBUG*/
                                     ...
                                     unsigned int* addr)
{
    ...
    if (!f  ||  !*f)
        return 0;
    r = 0;
    external = !(s = strdup(f + strspn(f, " \t"))) ? 1 : 0;
    for (p = s;  p  &&  *p;  p += strspn(p, ", \t")) {
        q = p + strcspn(p, ", \t");
        if (*q) {
            *q++ = '\0';
        }
        if (*p  &&  (ip = SOCK_gethostbyname(p)) != 0) {
            int/*bool*/ private_ip;
            if (!(private_ip = s_IsPrivateIP(ip))  &&  !NcbiIsLocalIP(ip)) {
                r     = p;
                *addr = ip;
                break;
            } else if (!r  ||  (!private_ip  &&  s_IsPrivateIP(*addr))) {
                r     = p;
                *addr = ip;
            }
        } else {
            /* unresolvable name causes external mapping */
            external = 1;
        }
        p = q;
    }
...
/* Environment checked here must be in correspondence with
 * tracking environment created in CTrackingEnvHolder::CTrackingEnvHolder()
 * (header: <cgi/ncbicgi.hpp>, source: cgi/ncbicgi.cpp, library: cgi)
 */
                                         ...
                                         const char* const* tracking_env)
{
    ...
    for (i = 0;  i < sizeof(probe)/sizeof(probe[0]);  i++) {
        ...
        case 1:
            ...
            break;
        case 2:
            probe[i].host = s_SearchTrackingEnv("PROXIED_IP", tracking_env);
            break;
        case 3:
        ...
        if (NcbiIsLocalIP(probe[i].ip)) {
            continue;
        }
        external = 1;
        if (!s_IsPrivateIP(probe[i].ip)) {
            host = probe[i].host;
            ip   = probe[i].ip;
            break;
        }
